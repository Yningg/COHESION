...
def calcThresholdSIT(PI, NI, node_mutual_set, key, t_obs, method, rate):
    ...
    uME_pos = getPairEdges(PI, u, list(NI[u][1]), trim=True, sort=True)
    vME_pos = getPairEdges(PI, v, list(NI[v][1]), trim=True, sort=True)
...
def findBoundsPANE(trimmed_index, mutual_nodes, t_obs, last_mutual_key, last_key, method, rate):
    ...
    node_set, EI_threshold = calcThresholdEI(trimmed_PI_pos, trimmed_NI, node_set, last_key, t_obs, method, rate)
    mutual_nodes, SIT_threshold = calcThresholdSIT(trimmed_PI_pos, trimmed_NI, mutual_nodes, last_mutual_key, t_obs, method, rate)
    ...
    for u in tqdm(node_set, desc="Calculating EI boundaries for each user"):
        uE_pos, uE_count = getEdges(trimmed_PI_pos, u, trimmed_NI[u][0] | trimmed_NI[u][1], trim=True)
    ...
    for u in tqdm(mutual_nodes, desc="Calculating SIT boundaries for each user"):
        uME_pos = getPairEdges(trimmed_PI_pos, u, list(trimmed_NI[u][1]), trim=True, sort=True)
    ...
    minValues, maxValues = getGIDBounds(trimmed_PI_pos, t_obs, minValues, maxValues, trim=True)
...
if __name__ == "__main__":
    ...
    node_mapping_file = "./Datasets/Node_Mapping/BTW_node_mapping.txt"
    node_mapping = read_node_mapping(node_mapping_file)
    pro_dataset, t_obs = preprocess_dataset("./Datasets/OSNs/BTW_attributed.txt", "BTW", node_mapping, t_obs)
    ...
    LB_values, UB_values = findBoundsPANE(trimmed_index, mutual_nodes, t_obs, last_mutual_key, last_key, decay_method, decay_rate)
